import { useState } from "react";//react 모듈 중에서 useState만 골라 불러오겠다

function Exam01 () {
    //데이터
    //- const는 값이 변하지 않는 변수를 의미
    //- React는 화면을 자동으로 갱신해주기 때문에 불변값이 아니면 처리해주지 않음
    //- 변경은 setter 메소드로 처리하도록 설정되어 있음
    //- 변수를 선언할 때 setter 메소드까지 같이 만들도록 useState(상태변수)라는 명령을 제공함
