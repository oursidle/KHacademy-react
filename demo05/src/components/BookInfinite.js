...
//사용법 : throttle(원래 사용하려고 했던 함수, 주기)
//import debounce from "lodash/debounce";//특정 이벤트의 마지막만 실행하도록 설정(강제 성능 향상)(아이디 검사, ...)
//사용법 : debounce(원래 사용하려고 했던 함수, 주기)
...
const BookInfinite = (props) => {
    ...
    //개수가 변하면 페이지를 1로, 목록을 모두 지우고 불러와야 함
    useEffect(() => {
        setPage(1);
        setBookList([]);
        loadBook();
    ...
    //////////////////////////////////////////////////////////////////////
    useEffect(() => {
        if(loading.current === true){//로딩이 진행 중인 상황
            return;//그만
        }
